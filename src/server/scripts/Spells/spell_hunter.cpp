...
enum HunterSpells
{
	...
	SPELL_HUNTER_A_MURDER_OF_CROWS_SUMMON			= 129179,
	SPELL_HUNTER_GLAIVE_TOSS						= 117050,
	SPELL_HUNTER_GLAIVE_TOSS_AURA					= 117050,
	SPELL_HUNTER_GLAIVE_TOSS_DAMAGES				= 121414,
	...
	SPELL_HUNTER_NARROW_ESCAPE_RETS					= 128405,
        ...
        class spell_hun_a_murder_of_crows_AuraScript : public AuraScript
        {
            ...
            void OnTick(const AuraEffect* aurEff)
            {
                if (Unit* target = GetTarget())
                {
                    ...
                    if (Player* _player = GetCaster()->ToPlayer())
                    {
                        _player->CastSpell(target, SPELL_HUNTER_A_MURDER_OF_CROWS_SUMMON, true);
            ...
            for (std::list<Creature*>::iterator itr = crowsList.begin(); itr != crowsList.end(); itr++)
    ...
    public:
        ...
        class spell_hun_disengage_SpellScript : public SpellScript
        {
            ...
            void HandleAfterCast()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->HasAura(SPELL_HUNTER_POSTHASTE))
                    {
                        _player->RemoveMovementImpairingAuras();
                        _player->CastSpell(_player, SPELL_HUNTER_POSTHASTE_INCREASE_SPEED, true);
                    }
                    else if (_player->HasAura(SPELL_HUNTER_NARROW_ESCAPE))
                    {
                        ...
                        for (std::list<Unit*>::iterator itr = retsList.begin(); itr != retsList.end(); itr++)
                            _player->CastSpell(*itr, SPELL_HUNTER_NARROW_ESCAPE_RETS, true);
                    }
                }
            }
