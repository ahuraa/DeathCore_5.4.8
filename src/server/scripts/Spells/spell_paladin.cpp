...
enum Illumination
{
	SPELL_PALADIN_MASTERY_ILLUMINATED_HEALING = 76669,
	SPELL_PALADIN_ILLUMINATED_HEALING = 86273
};

class spell_pal_mastery_illuminated_healing : public SpellScriptLoader
{
public:
	spell_pal_mastery_illuminated_healing() : SpellScriptLoader("spell_pal_mastery_illuminated_healing") { }

	class spell_pal_mastery_illuminated_healing_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_pal_mastery_illuminated_healing_AuraScript);

		bool CheckIllumination(ProcEventInfo& info)
		{
			if (!info.GetProcTarget() || !info.GetDamageInfo() || info.GetDamageInfo()->GetDamageType() != HEAL || !info.GetDamageInfo()->GetSpellInfo())
				return false;

			switch (info.GetDamageInfo()->GetSpellInfo()->Id)
			{
		...
		void HandleIllumination(ProcEventInfo& info)
		{
			int32 heal = int32(info.GetDamageInfo()->GetDamage() * (GetCaster()->ToPlayer()->GetMasterySpellCoefficient() / 100.0f));

			if (AuraEffect* aurShield = info.GetProcTarget()->GetAuraEffect(SPELL_PALADIN_ILLUMINATED_HEALING, EFFECT_0, GetCaster()->GetGUID()))
				heal += aurShield->GetAmount();

			int32 maxHealth = GetCaster()->CountPctFromMaxHealth(33);
			heal = std::min(heal, maxHealth);

			GetCaster()->CastCustomSpell(info.GetProcTarget(), SPELL_PALADIN_ILLUMINATED_HEALING, &heal, NULL, NULL, true);
		}

		void Register()
		{
			DoCheckProc += AuraCheckProcFn(spell_pal_mastery_illuminated_healing_AuraScript::CheckIllumination);
			OnProc += AuraProcFn(spell_pal_mastery_illuminated_healing_AuraScript::HandleIllumination);
		}
	};

	AuraScript* GetAuraScript() const
	{
		return new spell_pal_mastery_illuminated_healing_AuraScript();
	}
};

void AddSC_paladin_spell_scripts()
{
	new spell_pal_mastery_illuminated_healing();
